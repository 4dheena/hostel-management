...
/* GET WARDEN HOSTEL */
$stmt = $conn->prepare("SELECT hostel_id FROM wardens WHERE user_id = ?");
...
<td><?= $row['stay_from'] ?> → <?= $row['stay_to'] ?></td>
<td>
<?php if(!empty($row['id_proof_path'])): ?>
<a href="../<?= $row['id_proof_path'] ?>" target="_blank">View</a>
<?php else: ?>None<?php endif; ?>
</td>
<td>
<button class="view-btn" onclick="openGuestModal(<?= $row['id'] ?>)">Review</button>
</td>
</tr>
<?php endwhile; ?>
...
$res_a = mysqli_query($conn,"SELECT name FROM students WHERE student_id='{$srow['student_a_id']}'");
$res_b = mysqli_query($conn,"SELECT name FROM students WHERE student_id='{$srow['student_b_id']}'");
...
<p>
<b><?= $a_data['name'] ?></b> (Room <?= $srow['room_a'] ?>)
<br>⬇⬆<br>
<b><?= $b_data['name'] ?></b> (Room <?= $srow['room_b'] ?>)
</p>

<p><b>Reason:</b> <?= htmlspecialchars($srow['reason']) ?></p>

<button onclick="approveSwap(<?= $srow['id'] ?>)" class="approve">Approve</button>
<button onclick="rejectSwap(<?= $srow['id'] ?>)" class="reject">Reject</button>
...
<button type="button" class="reject" onclick="showReject()">Reject</button>

<div id="rejectBox" style="display:none">
<label>Reason:</label>
<textarea name="reject_reason"></textarea>
<br><br>
<button class="reject" name="decision" value="rejected">Submit</button>
</div>
...
SELECT name FROM students WHERE student_id='{$row['student_id']}'
"));
?>
...
<p>
<b><?= $student['name']; ?></b> wants to vacate room
</p>

<p><b>Vacate Date:</b> <?= $row['vacate_date']; ?></p>
<p><b>Reason:</b> <?= $row['reason']; ?></p>

<button onclick="approveVacate(<?= $row['id']; ?>)" class="approve">
Approve
</button>

<button onclick="rejectVacate(<?= $row['id']; ?>)" class="reject">
Reject
</button>
...
<p><b>Stay:</b> ${data.stay_from} to ${data.stay_to}</p>
`;
...
function closeModal(){ document.getElementById("guestModal").style.display="none"; }

function showReject(){ document.getElementById("rejectBox").style.display="block"; }

function filterRequests(){
let type=document.getElementById("filterType").value;
document.getElementById("guestSection").style.display=(type=="guest")?"block":"none";
document.getElementById("swapSection").style.display=(type=="swap")?"block":"none";
}

function approveSwap(id){
if(confirm("Approve this swap?")){
window.location.href="process_swap.php?action=approve&id="+id;
}
}
...
if(reason){
window.location.href="process_swap.php?action=reject&id="+id+"&reason="+encodeURIComponent(reason);
}
}
function approveVacate(id){
if(confirm("Approve this vacate request?")){
window.location.href="../vacate/process_vacate.php?action=approve&id="+id;
}
}
...
let reason = prompt("Enter rejection reason:");
if(reason){
window.location.href="../vacate/process_vacate.php?action=reject&id="+id+"&reason="+encodeURIComponent(reason);
}
}
